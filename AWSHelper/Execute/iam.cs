...
namespace AWSHelper
{
    ...
    {
        private static void executeIAM(string[] args)
        {
            var nArgs = CLIHelper.GetNamedArguments(args);

            var helper = new IAMHelper();
            switch (args[1])
            {
                case "create-policy":
                    {
                        ...
                        var result = helper.CreateAdminAccessPolicyS3Async(
                            name: nArgs["name"],
                            path: nArgs["path"],
                            description: nArgs.FirstOrDefault(x => x.Key == "description").Value ?? $"Admin S3 Access Policy to '{nArgs["path"]}' auto generated by AWSHelper").Result;

                        Console.WriteLine($"SUCCESS, {nArgs["name"]} policy was created for path {nArgs["path"]}, Arn: {result.Policy.Arn}.");
                    }
                    ; break;
                case "delete-policy":
                    {
                ...
                case "help": HelpPrinter($"{args[0]}", "Amazon Identity and Access Management",
                    ("create-admin-access-policy-s3", "Accepts params: type ('admin-access-s3'), name, path, description (optional)"),
